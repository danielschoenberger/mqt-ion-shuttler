...
# This version runs a syndrome-extraction round (surface code) instead of a QASM circuit.
...
from mqt.ionshuttler.multi_shuttler.outside.graph_utils import get_idx_from_idc
...
from mqt.ionshuttler.multi_shuttler.outside.syndrome_compilation import (
...
SAVE_IMAGES = False
SYNDROME_DISTANCE = 5    # surface code distance
SYNDROME_PZ_STRATEGY = "geometric"  # "geometric" or "round_robin"
OUTPUT_JSON = "timeline.json"

# Paths/Cycles mode string for new main signature
CYCLE_OR_PATHS_STR = "cycles"  # allowed values include "cycles", "paths", and "hybrid"
...
def run_single(
    ...
    syndrome_distance: int = SYNDROME_DISTANCE,
) -> Dict[str, Any]:
    _COLLECTOR.frames.clear()
    _COLLECTOR.q_map = None
    _COLLECTOR.pz_index = None
    _COLLECTOR.name_to_side = {}

    height = -4.5

    # PZ anchor points (unchanged from original)
    into_anchor_1 = (float((m - 1) * v), float((n - 1) * h))
    out_anchor_1 = (float((m - 1) * v), float(0))
    processing_zone1 = (float((m - 1) * v - height), float((n - 1) * h / 2))

    into_anchor_2 = (0.0, float(0))
    out_anchor_2 = (0.0, float((n - 1) * h))
    processing_zone2 = (float(height), float((n - 1) * h / 2))

    into_anchor_3 = (float((m - 1) * v), float(0))
    out_anchor_3 = (float(0), float(0))
    processing_zone3 = (float((m - 1) * v / 2), float(height))

    into_anchor_4 = (float(0), float((n - 1) * h))
    out_anchor_4 = (float((m - 1) * v), float((n - 1) * h))
    ...
    # graphs
    basegraph_creator = GraphCreator(m, n, v, h, failing_junctions=0, pz_info=pzs, seed=seed)
    MZ_graph = basegraph_creator.get_graph()

    pzgraph_creator = PZCreator(m, n, v, h, failing_junctions=0, pzs=pzs, seed=seed, mid_side_pz_connections=True)
    ...
    # --- Syndrome Scheduler Setup ---
    print(f"Using syndrome scheduler (distance={syndrome_distance})")

    _data_qubits, _ancilla_qubits, plaquettes = build_surface_code_pattern(syndrome_distance)
    ...
    # Derive number of ions from all qubits involved
    ...
    number_of_chains = len(all_qubit_ids)
    print(f"Syndrome qubits (data + ancilla): {number_of_chains}")
    ...
    G.sequence = create_initial_syndrome_sequence(syndrome_state)
    print(f"Number of syndrome gates: {len(G.sequence)}")

    # Plaquette-to-PZ partitioning
    if len(G.pzs) == 1:
        map_to_pz: Dict[int, str] = {qid: G.pzs[0].name for qid in all_qubit_ids}
    else:
        plaquette_to_pz = assign_plaquettes_to_pzs(
            plaquettes, shared_boundaries, G, strategy=SYNDROME_PZ_STRATEGY,
        )
    ...
    G.syndrome_load_aware = SYNDROME_PZ_STRATEGY == "geometric"
    print(f"Syndrome map_to_pz: {map_to_pz}")

    # Reorder sequence using distances
    G.locked_gates = {}
    G.sequence = create_updated_syndrome_sequence(G, syndrome_state)
    print(f"Reordered syndrome sequence length: {len(G.sequence)}")
    ...
    # Run the shuttler with syndrome_state (no DAG)
    _ = run_shuttle_main(
        G, None, CYCLE_OR_PATHS_STR, use_dag=False, syndrome_state=syndrome_state
    )
    ...
    out = {
        ...
        "pzs": sides_present,
        "meta": {
            "algorithm": f"syndrome_d{syndrome_distance}",
            "seed": seed,
            "arch": [m, n, v, h],
