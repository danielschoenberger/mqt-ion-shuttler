...
def write_viz_json(
    *,
    solver: SyndromeSchedulerLayered,
    model,
    filename: str,
    m: int,
    n: int,
...
class SyndromeSchedulerLayered:
    """Strict Tomita-Svore layered schedule: no commutation freedom.

    Z-plaquettes (row sweep): TL=layer 1, TR=2, BL=3, BR=4.
    X-plaquettes (col sweep): TL=layer 1, BL=2, TR=3, BR=4.
    Weight-2 boundaries inherit the same corner->layer map.

    Enforced as a global barrier: for every (k, l) with layer(k) < layer(l),
    gate l starts at least `gate_duration` after gate k. Correctness on shared
    qubits follows from the Tomita-Svore layer choice (every mixed-type
    adjacent pair touches each shared qubit in different layers with
    consistent ordering).
    """
    def __init__(
        self, 
        ...
        # Per-plaquette structural info: {"ancilla_id", "type", "weight", "label_to_gate"}
        self.plaquettes: list[dict] = []
        # ancilla_ion_id -> "Z" or "X"
        self.ancilla_type: dict[int, str] = {}
        # Tomita-Svore layer assignment per gate: gate_idx -> int in {1,2,3,4}
        self.gate_layer: dict[int, int] = {}
    ...
    def generate_workload(self) -> None:
        ...
        for i, edge in logical_pos_map.items():
            if self.ion_kind[i] == "anc":
                ...
                # Tomita-Svore corner -> layer map (1-indexed, distinct across type)
                layer_map = (
                    {"TL": 1, "TR": 2, "BL": 3, "BR": 4}  # Z: row sweep
                    if stype == "Z"
                    else {"TL": 1, "BL": 2, "TR": 3, "BR": 4}  # X: col sweep
                )

                # Emit gates by corner label, pin each gate to its layer
                ...
                for (did, label, _) in neighbors:
                    ...
                    self.gate_layer[gate_idx] = layer_map[label]

                self.plaquettes.append({
                    "ancilla_id": i,
                    "type": stype,
                    "weight": len(neighbors),
                    "label_to_gate": label_to_gate,
                })

        layer_counts = {k: sum(1 for v in self.gate_layer.values() if v == k) for k in (1, 2, 3, 4)}
        print(f"  > Tomita-Svore layer counts: {layer_counts}")
    ...
    def _add_gate_constraints(self) -> None:
        ...
        # --- Global Tomita-Svore layer barrier ---
        # For every ordered pair of gates (k, l) with layer(k) < layer(l):
        # gate l may not start before gate k has finished.
        # This single family subsumes both intra-plaquette ordering and
        # cross-plaquette shared-qubit consistency.
        items = list(self.gate_layer.items())
        for i_idx in range(len(items)):
            ...
            for j_idx in range(len(items)):
                ...
                l, Ll = items[j_idx]
                if Lk < Ll:
                    self.s.add(self.gate_time[l] >= self.gate_time[k] + self.gate_duration)

    def validate_solution(self) -> bool:
        """Post-solve sanity check: verify the global layer barrier holds."""
        ...
        for k in range(len(self.gates)):
            for t in range(self.timesteps):
                if bool(self.model.evaluate(self.gate_start[t][k])):
                    ...
                    break
        ok = True
        for k, Lk in self.gate_layer.items():
            for l, Ll in self.gate_layer.items():
                if Lk < Ll and gt[l] < gt[k] + self.gate_duration:
                    print(f"  [validate] layer-barrier violation: gate {k} (layer {Lk}, t={gt[k]}) vs gate {l} (layer {Ll}, t={gt[l]})")
                    ok = False
        layer_times = {L: sorted(gt[k] for k, Lk in self.gate_layer.items() if Lk == L) for L in (1, 2, 3, 4)}
        print(f"  [validate] per-layer start times: {layer_times}")
        if ok:
            print("  [validate] layer barrier holds on all gate pairs.")
        return ok
...
if __name__ == "__main__":
    ...
    sched = SyndromeSchedulerLayered(
